...
public class lb_BirdController : MonoBehaviour {
	...
	bool pause = false;
    public List<GameObject> myBirds;
	...
	void Start () {
        ...
        myBirds = new List<GameObject>();
		GameObject bird;
		for(int i=0;i<maximumNumberOfBirds;i++){
			if(highQuality){
            ...
            //GameObject mybird = Instantiate(bird, Vector3.zero, Quaternion.identity) as GameObject;
            //myBirds.Add(mybird);
			//mybird.transform.localScale = mybird.transform.localScale*birdScale;
			//mybird.transform.parent = transform;
			//mybird.SendMessage ("SetController",this);
	...
	void UpdateBirds(){
        ...
        //oooooh this rotates back around to 0...
        birdIndex++;
	}
	...
	void SpawnBird(){
		if (!pause){
			...
			int randomBirdIndex = Mathf.FloorToInt (Random.Range (0,maximumNumberOfBirds));
			int loopCheck = 0;
			...
			while(bird == null){
				...
				randomBirdIndex = randomBirdIndex+1 >= maximumNumberOfBirds ? 0:randomBirdIndex+1;
				loopCheck ++;
				if (loopCheck >= maximumNumberOfBirds){
    ...
    public void SpawnSpecificBird(string birdType)
    {
        if (!pause)
        {
            GameObject bird;
            bird = Resources.Load(birdType + "HQ", typeof(GameObject)) as GameObject;
            GameObject mybird = Instantiate(bird, Vector3.zero, Quaternion.identity) as GameObject;
            myBirds.Add(bird);
            mybird.transform.localScale = mybird.transform.localScale*birdScale;
            mybird.transform.parent = transform;
            mybird.SendMessage ("SetController",this);
            BirdFindTarget(mybird);
        }
    }
